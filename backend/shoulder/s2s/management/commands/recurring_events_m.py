        ...
        try:
            ...
            for event in events:
                ...
                time_of_day = datetime.datetime.strptime(
                    event["time_of_day"], "%H:%M"
                ).time()
                event_date = self.get_next_weekday(
                    datetime.datetime.now(), time_of_day, day_of_week
                )
            ...
            self.stdout.write(
                self.style.ERROR("Error importing data: {}".format(str(e)))
            )
